...
class Change {
  ...
  async get(path, {
    ...
    let {
      ...
      leaf,
      leafName
    } = await this.root.fileAtPath(path, !!allowCreate);
  ...
  async _makeCommit(commitOpts) {
    ...
    let commitOid = await git_1.Commit.create(this.repo, commitOpts, tree, this.parents);
    return git_1.Commit.lookup(this.repo, commitOid);
  }
  async _pushCommit(mergeCommit) {
    const remoteBranchName = `temp-remote-${crypto_1.randomBytes(20).toString('hex')}`;
    await this.repo.createBranch(remoteBranchName, mergeCommit);
    let remote = await this.repo.getRemote('origin');
    try {
      await remote.push(`refs/heads/${remoteBranchName}`, `refs/heads/${this.targetBranch}`, {
        force: true
      });
    } catch (err) {
  ...
  async _makeMergeCommit(newCommit, commitOpts) {
    ...
    let baseOid = await git_1.Merge.base(this.repo, newCommit.id(), headCommit.id());
    ...
    let mergeResult = await git_1.Merge.perform(this.repo, newCommit, headCommit, commitOpts);
