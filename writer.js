...
class Writer {
  ...
  async prepareCreate(session, type, document, isSchema) {
    ...
    return withErrorHandling(id, type, async () => {
      ...
      let change = await change_1.create(this.repo, null, this.branchPrefix + defaultBranch, !!this.remote);
      let file;
      while (id == null) {
        let candidateId = this._generateId();
        let candidateFile = await change.get(this._filenameFor(type, candidateId, isSchema), {
          allowCreate: true
        });
  ...
  async prepareUpdate(session, type, id, document, isSchema) {
    ...
    await this._ensureRepo();
    return withErrorHandling(id, type, async () => {
      let change = await change_1.create(this.repo, meta.version, this.branchPrefix + defaultBranch, !!this.remote);
  ...
  async _ensureGithereum() {
    ...
    if (!this.githereum && this.githereumConfig) {
      ...
      this.githereum = new Githereum(this.repo.path, this.githereumConfig.repoName, contract, this.githereumConfig.from, {
